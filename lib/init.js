...
async function checkDependencies(rl) {
  ...
  if (process.env.ANTHROPIC_API_KEY) {
    ...
    if (apiKey && apiKey.trim()) {
      const envDir = path.join(os.homedir(), '.antenna');
      const envFile = path.join(envDir, 'env');
      fs.mkdirSync(envDir, { recursive: true });

      // Read existing env file and update/append the key
      let envContent = '';
      ...
      console.log('');
      console.log('  To persist across sessions, add this to your shell profile (~/.zshrc or ~/.bashrc):');
      console.log(`    source ${envFile}`);
      console.log('');
      console.log('  Or export for this session:');
      console.log(`    export ANTHROPIC_API_KEY=${apiKey.trim()}`);
    } else {
      warn('No API key entered — antenna run will fail without it.');
      info('Set ANTHROPIC_API_KEY in your environment or re-run: antenna init');
    }
  }
}
