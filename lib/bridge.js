...
/**
 * Build environment variable pass-through instructions for daemon templates.
 *
 * SECURITY: We do NOT bake credential values into daemon config files.
 * Instead, we document which env vars must be set in the user's shell profile.
 * The bash bridge script reads passwords from env vars at runtime.
 *
 * For launchd: env vars must be set via launchctl setenv or shell profile.
 * For systemd: env vars should be in ~/.config/environment.d/ or the unit's EnvironmentFile.
 */
function buildEnvVars() {
  // No env vars need to be baked into templates.
  // The bridge scripts read credentials from env vars at runtime
  // using the names specified in config (imap_pass_env, smtp_pass_env).
  return '';
}
