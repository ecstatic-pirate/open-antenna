...
/**
 * Check that Claude is authenticated — either via OAuth credentials file or ANTHROPIC_API_KEY.
 * Returns an error string if not authenticated, null if OK.
 */
function checkClaudeAuth() {
  // API key takes precedence — if set, Claude will use it
  if (process.env.ANTHROPIC_API_KEY) return null;

  // Linux: credentials in file
  ...
  if (process.platform === 'darwin') {
    ...
    if (result.status === 0 && result.stdout?.includes('claudeAiOauth')) {
      return null; // authenticated via macOS Keychain
    }
  }

  return "Claude not authenticated. Run 'claude login' first (or set ANTHROPIC_API_KEY).";
}
