...
const { checkBin, todayString } = require('./utils');
...
/**
 * Source ~/.antenna/env if it exists: parse KEY=VALUE lines into process.env.
 * Called before any Claude invocation so the API key is available.
 */
function sourceAntennaEnv() {
  const envFile = path.join(os.homedir(), '.antenna', 'env');
...
/**
 * Check that required external deps are available, including ANTHROPIC_API_KEY.
 */
function checkDeps() {
  const errors = [];
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    errors.push("ANTHROPIC_API_KEY not set. Run 'antenna init' to configure.");
  }
...
async function run(options = {}) {
  ...
  // Source ~/.antenna/env before dependency checks so ANTHROPIC_API_KEY is available
  sourceAntennaEnv();
